...
class _ArmScope:
    ...
    def plan_to_configuration(self, q_goal, **kwargs):
        ...
        return self._robot._arm_group.plan_to_configuration({self._side: q_goal}, **kwargs)
    ...
    def check_collisions(self):
        ...
        return self._robot._arm_group.check_collisions(self._side)
...
class Openarm:
    ...
    def get_arm_spec(self, arm: Arm) -> OpenarmArmSpec:
        ...
        if arm is self._arm_group.arms["left"]:
    ...
    def _resolve_arms(self, arm: Arm | str | None) -> list[Arm]:
        ...
        if arm is None:
            return [self._arm_group.arms["right"], self._arm_group.arms["left"]]
        if isinstance(arm, str):
            if arm in ("left", "left_arm"):
                return [self._arm_group.arms["left"]]
            if arm in ("right", "right_arm"):
                return [self._arm_group.arms["right"]]
    ...
    def go_to(self, pose_name: str, ctx: SimContext | None = None) -> bool:
        """Move the named arm(s) to a named configuration, planned JOINTLY so
        they're validated against every other arm along the whole path.
        ...
        Returns:
            True if the joint plan succeeded and every named arm executed.
        """
        ...
        goal = {side: np.array(q) for side, q in pose.items()}

        path = self._arm_group.plan_to_configuration(goal)
        if path is None:
            logger.warning("Failed to jointly plan arms to '%s'", pose_name)
            return False

        combined_traj = self._arm_group.retime(path)
        per_arm_traj = self._arm_group.split_trajectory(combined_traj)

        success = True
        for side in pose:
            if side not in per_arm_traj:
                logger.warning("Unknown arm '%s' in named pose '%s'", side, pose_name)
                success = False
                continue
            if not ctx.execute(per_arm_traj[side]):
                logger.warning("Failed to execute %s arm trajectory for '%s'", side, pose_name)
                success = False
        return success
    ...
    def _plan_with_sequence(
        ...
        *,
        ...
        random.shuffle(arms)  # candidate order fairness

        heights = base_heights if base_heights else [None]
        sequence = [(a, h) for h in heights for a in arms]
    ...
    def setup_scene(
        ...
        if "ready" in self._named_poses:
            for side, arm in self._arm_group.arms.items():
