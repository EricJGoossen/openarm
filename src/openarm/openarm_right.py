...
"""OpenArm (right) 7-DOF arm constants and factory.
...
OPENARM_RIGHT_JOINT_NAMES = [f"openarm_right_joint{i}" for i in range(1, 8)]
...
OPENARM_RIGHT_VELOCITY_LIMITS = np.array([16.754666, 16.754666, 5.445426, 5.445426, 20.943946, 20.943946, 20.943946]) * SAFETY_SCALE

# Acceleration limits — not published; derived from v_max / 0.1s
OPENARM_RIGHT_ACCELERATION_LIMITS = np.array([167.54666, 167.54666, 54.45426, 54.45426, 209.43946, 209.43946, 209.43946]) * SAFETY_SCALE
...
OPENARM_RIGHT_LOWER = np.array([-1.396263, -0.17453267320510335, -1.570796, 0.0, -1.570796, -0.785398, -1.570796])
OPENARM_RIGHT_UPPER = np.array([3.490659, 3.3161253267948965, 1.570796, 2.443461, 1.570796, 0.785398, 1.570796])


# Effort limits (N·m) from URDF.
OPENARM_RIGHT_EFFORT_LIMITS = np.array([40.0, 40.0, 27.0, 27.0, 7.0, 7.0, 7.0])
...
OPENARM_RIGHT_HOME = np.zeros(7)

# Gripper travel limits (meters, per URDF).
OPENARM_RIGHT_FINGER_CLOSED = 0.0   # meters — fully closed per URDF limit
OPENARM_RIGHT_FINGER_OPEN = 0.044   # meters — fully open per URDF limit
...
def create_openarm_right_arm(
   env: "Environment",
   *,
   ee_site: str = "openarm_right_ee_site",
   with_ik="auto",
   extra_arm_body_names: list[str] | None = None,
   grasp_manager=None,
) -> "Arm":
   """Create an OpenArm (right) arm with analytical IK via ssik.
   ...
   from openarm import openarm_right_ik


   config = ArmConfig(
       name="openarm_right",
       entity_type="arm",
       joint_names=list(OPENARM_RIGHT_JOINT_NAMES),
       kinematic_limits=KinematicLimits(
           velocity=OPENARM_RIGHT_VELOCITY_LIMITS.copy(),
           acceleration=OPENARM_RIGHT_ACCELERATION_LIMITS.copy(),
       ),
       ee_site=ee_site,
   ...
   arm = Arm(env, config)
   ik_solver = resolve_ik_solver(arm, with_ik=with_ik, ssik_module=openarm_right_ik)
