...
def parse_args():
    ...
    p.add_argument("--position-tolerance-rad", type=float, default=0.02)
...
def load_or_generate_session(robot, args, rng) -> list[dict]:
    ...
    for i in range(6):
        if i % 3 == 2:
            ops.append(
                {
                    "type": "bimanual",
                    "goal_left": (getattr(robot, "left").arm.get_joint_positions() + rng.uniform(-args.goal_delta_rad, args.goal_delta_rad, 7)).tolist(),
                    "goal_right": (getattr(robot, "right").arm.get_joint_positions() + rng.uniform(-args.goal_delta_rad, args.goal_delta_rad, 7)).tolist(),
                }
            )
        else:
            ...
            ops.append(
                {
                    "type": "single",
                    "arm": arm,
                    "goal": (getattr(robot, arm).arm.get_joint_positions() + rng.uniform(-args.goal_delta_rad, args.goal_delta_rad, 7)).tolist(),
                }
            )
    return ops
...
def main():
    ...
    operations = load_or_generate_session(robot, args, rng)
